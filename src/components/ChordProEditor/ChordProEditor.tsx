...
interface ChordProEditorProps {
  ...
  onSwipePrev?: () => void; // Called when user swipes right (go to previous song)
...
type ChordProAPIBound = {
  ...
  setStyles: (styles: Settings["chordProStyles"] | null, draw?: boolean) => void;
  handleExternalChordBoxTouch: (event: MouseEvent, down: boolean, showChordDialog?: boolean) => boolean;
  isReadOnly: () => boolean;
...
class ChordProEditor extends React.Component<ChordProEditorProps, ChordProEditorState> {
  ...
  private prevPageRef: HTMLDivElement | null = null;
  private nextPageRef: HTMLDivElement | null = null;
  private prevHost: HTMLDivElement | null = null;
  ...
  componentDidUpdate(prevProps: ChordProEditorProps) {
    ...
    // Keep the pre-rendered prev/next pages in sync. Force a re-render of the
    // neighbours when display-affecting settings change.
    if (
      prevProps.settings?.chordProStyles !== this.props.settings?.chordProStyles ||
      prevProps.settings?.hideChordsInReadonlyEditor !== this.props.settings?.hideChordsInReadonlyEditor ||
      prevProps.forceThemeMode !== this.props.forceThemeMode
    ) {
      this.loadedPrevKey = null;
      this.loadedNextKey = null;
    }
  ...
  private setCurrentPageRef = (el: HTMLDivElement | null) => {
    this.currentPageRef = el;
  };

  private setPrevPageRef = (el: HTMLDivElement | null) => {
    this.prevPageRef = el;
  };

  private setNextPageRef = (el: HTMLDivElement | null) => {
    this.nextPageRef = el;
  };

  private setPrevHostRef = (el: HTMLDivElement | null) => {
    if (this.prevHost && this.prevHost !== el) this.getBoundChordProAPI(this.prevHost)?.dispose?.();
    this.prevHost = el;
    this.loadedPrevKey = null;
    if (el) this.loadNeighborPages();
  };

  private setNextHostRef = (el: HTMLDivElement | null) => {
    if (this.nextHost && this.nextHost !== el) this.getBoundChordProAPI(this.nextHost)?.dispose?.();
    this.nextHost = el;
    this.loadedNextKey = null;
    if (el) this.loadNeighborPages();
  };
  ...
  private installPinchScrollModeHandler(element: HTMLElement) {
    ...
    this.pinchTouchHandler = (e: TouchEvent) => {
      if (e.type === "touchstart") {
        if (this.pinchState && e.touches.length < 2) finishPinch();
    ...
    element.addEventListener("touchstart", this.pinchTouchHandler, opts);
    element.addEventListener("touchmove", this.pinchTouchHandler, opts);
    element.addEventListener("touchend", this.pinchTouchHandler, opts);
    element.addEventListener("touchcancel", this.pinchTouchHandler, opts);
  }
  ...
  private loadNeighborPage(which: "prev" | "next", host: HTMLDivElement | null, song: Song | null) {
    if (!host) return;
    const key = song ? `${song.Id} ${song.Text}` : null;
    const loadedKey = which === "prev" ? this.loadedPrevKey : this.loadedNextKey;
    if (key === loadedKey) return;
    if (which === "prev") this.loadedPrevKey = key;
    else this.loadedNextKey = key;
    ...
    api.setStyles?.(this.props.settings?.chordProStyles ?? null);
    const isDark = this.props.forceThemeMode ? this.props.forceThemeMode === "dark" : document.documentElement.getAttribute("data-theme") === "dark";
    api.darkMode?.(isDark);
