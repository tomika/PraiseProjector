/**
 * Configurable client-view input contract: the action vocabulary, the persisted binding
 * profile shape, and the pure logic that interprets both.
 *
 * This lives in `common/` because the profile is part of the persisted `Settings` object,
 * which crosses the renderer / Electron main / webserver boundary. Nothing here may touch
 * the DOM — the keyboard matcher needs `KeyboardEvent` and the `chordpro/keycodes`
 * normalizer, so it stays in `src/client-view/input/clientViewInput.ts`, which re-exports
 * this module so callers keep a single import site.
 */
