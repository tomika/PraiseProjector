 ...
 * Code debug session serves the freshly-built client instead of a stale bundle,
 * and from `npm run build:web` for production.
 *
...
// build:web (vite --mode web → dist/web) writes the entry + hashed assets here.
const outputFile = path.join(projectRoot, "dist", "web", "client-view.html");
...
async function main() {
  ...
  console.log(`[client-view] output: ${fmt(outputMtime)}`);
  console.log(`[client-view] source: ${fmt(newestSourceMtime)}`);
  console.log("[client-view] Running: npm run build:web");
